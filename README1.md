...
Конфигурация эмулятора:
  VFS: tests/vfs_archives/complex_vfs.zip
  Стартовый скрипт: не указан
  Режим отладки: True
  Пользователь: IlyaRassylshchikov
  Хост: HOSTNAME
  VFS загружена: True
  VFS файлов: 16
  VFS директорий: 11
  Доступные команды: cat, cd, chown, exit, find, ls, pwd, rm, tree, vfs-info
============================================================
```

### Трассировка ошибок
При включенном `--debug` выводятся полные traceback для исключений.
...
### Версия 1.0.0 (2025-10-18)
- ✅ Завершены все 5 этапов
- ✅ Полная документация
- ✅ Все команды протестированы

### Версия 0.5.0 (Этап 5)
- Добавлены команды chown и rm
- Поддержка изменения владельцев файлов
- Рекурсивное удаление директорий

### Версия 0.4.0 (Этап 4)
- Полная реализация команд ls, cd, cat, find
- Поддержка паттернов поиска

### Версия 0.3.0 (Этап 3)
- Интеграция виртуальной файловой системы
- Загрузка VFS из ZIP
- Команды pwd, tree, vfs-info
